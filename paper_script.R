...
#setwd("C:/Users/martinsson/sciebo - Martinsson, Elin (emartins@uni-bonn.de)@uni-bonn.sciebo.de (2)/StormLab/Research/Elin/Paper3_1/R/paper_3")
getwd()
########################
...
# for ref, this data has more variables, if needed
#eurostat <- read.csv("used_data/agr_r_accts__custom_13034707_linear.csv.gz") 
...
#ppp_eurostat <- read.csv("used_data/ppp_eurostat.gz")
...
ggarrange(p_livestock, p_arable_crop, p_crop_high, ncol = 3)
...
# display the three plots at the same time
ggarrange(p_continental, p_atlantic, p_mediterranean, ncol = 3)
...
ggarrange(p_rural, p_intermediate, p_urban, ncol = 3)
...
# ---------------------------------------
ggarrange(p_livestock, p_arable_crop, p_crop_high, p_continental, p_atlantic, 
...
# ------------------------------------------------------------------------------
# robustness check
# with and without weight restrictions
# (Appendix 1)
# ------------------------------------------------------------------------------
...
# assess EE with no weight restrictions
ee_noweight <- dea(x_full, y_full, RTS = "crs", ORIENTATION = "in")
total_data_rm$ee_noweight <- ee_noweight$eff

# assess SE with no weight restrictions
se_noweight <- dea(x_full_s, y_full_s, RTS = "crs", ORIENTATION = "out")
total_data_rm$se_noweight <- 1 / se_noweight$eff


p_noweight_ee <- ggplot(total_data_rm, aes(x = eff_ee_lpsolve, y = ee_noweight)) +
  geom_point() +
  labs(title = "Robustness check EE", x = "Efficiency EE",
       y = "Efficiency EE without weight restriction") +
  theme_bw() +
  geom_point(size = 3) + aes(ymin=0)  + aes(xmin=0) +
  geom_text(aes(label = NUTS_ID), nudge_x = 0.01, nudge_y = 0.01) + 
  geom_abline(intercept = 0, slope = 1, color = "red")

p_noweight_se <- ggplot(total_data_rm, aes(x = eff_se_lpsolve, y = se_noweight)) +
  geom_point() +
  labs(title = "Robustness check SE", x = "Efficiency SE",
       y = "Efficiency SE without weight restriction") +
  theme_bw() +
  geom_point(size = 3) + aes(ymin=0)  + aes(xmin=0) +
  geom_text(aes(label = NUTS_ID), nudge_x = 0.01, nudge_y = 0.01) + 
  geom_abline(intercept = 0, slope = 1, color = "red")

ggarrange(p_noweight_ee, p_noweight_se)
# varying the environmental indicator used as benchmark
...
plot(total_data_rm$eff_ee_lpsolve, total_data_rm$ee_robust)
...
# overall efficiency EE and SE
ee_se_livestock <- subset(efficiency, select = c(eff_ee_livestock, eff_se_livestock))

# sum each row in ee_se except for the first row
ee_se_livestock$sum <- (ee_se_livestock$eff_ee_livestock + ee_se_livestock$eff_se_livestock)/ 2

plot(ee_se_livestock$sum)
...
# compare efficiency of different regions in different subsets

total_data_rm$eff_ee_meta <- total_data_rm$eff_ee_lpsolve
total_data_rm$eff_se_meta <- total_data_rm$eff_se_lpsolve

arable_crop$eff_ee_arable_crop <- arable_crop$eff_ee_lpsolve
arable_crop$eff_se_arable_crop <- arable_crop$eff_se_lpsolve
crop_high$eff_ee_crop_high <- crop_high$eff_ee_lpsolve
crop_high$eff_se_crop_high <- crop_high$eff_se_lpsolve
livestock$eff_ee_livestock <- livestock$eff_ee_lpsolve
livestock$eff_se_livestock <- livestock$eff_se_lpsolve

continental$eff_ee_continental <- continental$eff_ee_lpsolve
continental$eff_se_continental <- continental$eff_se_lpsolve
atlantic$eff_ee_atlantic <- atlantic$eff_ee_lpsolve
atlantic$eff_se_atlantic <- atlantic$eff_se_lpsolve
mediterranean$eff_ee_mediterranean <- mediterranean$eff_ee_lpsolve
mediterranean$eff_se_mediterranean <- mediterranean$eff_se_lpsolve

urban$eff_ee_urban <- urban$eff_ee_lpsolve
urban$eff_se_urban <- urban$eff_se_lpsolve
rural$eff_ee_rural <- rural$eff_ee_lpsolve
rural$eff_se_rural <- rural$eff_se_lpsolve
intermediate$eff_ee_intermediate <- intermediate$eff_ee_lpsolve
intermediate$eff_se_intermediate <- intermediate$eff_se_lpsolve

# create a dataframe containing the efficiency scores from all the above mentioned
# datasets

listall <- list(arable_crop, crop_high, livestock, continental, atlantic, mediterranean, 
                urban, rural, intermediate, total_data_rm)

efficiency <- listall %>% reduce(full_join, by=('NUTS_ID'))# %>% na.omit()

# select only the variables called something with eff

efficiency <- efficiency %>% select(NUTS_ID, eff_ee_arable_crop, eff_ee_crop_high, 
                                    eff_se_arable_crop, eff_se_crop_high, eff_ee_livestock, 
                                    eff_se_livestock, eff_ee_continental, 
                                    eff_se_continental, eff_ee_atlantic, eff_se_atlantic,
                                    eff_ee_mediterranean, eff_se_mediterranean, eff_ee_urban, eff_se_urban,
                                    eff_ee_rural, eff_se_rural, eff_ee_intermediate, eff_se_intermediate,
                                    eff_ee_meta, eff_se_meta, Typology.y, ls_dum.x, arable_crop_dum.x, 
                                    crop_high_dum.x,`Climatic Zone`)

### metatechnology ratio
# calculate the meta-technology ratio
efficiency$metatechnpology_ratio_ee_livestock <- efficiency$eff_ee_meta / efficiency$eff_ee_livestock
efficiency$metatechnpology_ratio_se_livestock <- efficiency$eff_se_meta / efficiency$eff_se_livestock

efficiency$metatechnpology_ratio_ee_arable_crop <- efficiency$eff_ee_meta / efficiency$eff_ee_arable_crop
efficiency$metatechnpology_ratio_se_arable_crop <- efficiency$eff_se_meta / efficiency$eff_se_arable_crop

efficiency$metatechnpology_ratio_ee_crop_high <- efficiency$eff_ee_meta / efficiency$eff_ee_crop_high
efficiency$metatechnpology_ratio_se_crop_high <- efficiency$eff_se_meta / efficiency$eff_se_crop_high

# technology gap
efficiency$tech_gap_ee_livestock <- 1 - efficiency$metatechnpology_ratio_ee_livestock
efficiency$tech_gap_se_livestock <- 1 - efficiency$metatechnpology_ratio_se_livestock

efficiency$tech_gap_ee_arable_crop <- 1 - efficiency$metatechnpology_ratio_ee_arable_crop
efficiency$tech_gap_se_arable_crop <- 1 - efficiency$metatechnpology_ratio_se_arable_crop

efficiency$tech_gap_ee_crop_high <- 1 - efficiency$metatechnpology_ratio_ee_crop_high
efficiency$tech_gap_se_crop_high <- 1 - efficiency$metatechnpology_ratio_se_crop_high

# EE: create a density plot of the tech_gap_ee_rural and the tech_gap_ee_intermediate
E <- ggplot(efficiency, aes(x = tech_gap_ee_livestock, fill = "Livestock")) +
  geom_density(alpha = 0.5) +
  geom_density(aes(x = tech_gap_ee_arable_crop, fill = "Arable Crop"), alpha = 0.5) +
  geom_density(aes(x = tech_gap_ee_crop_high, fill = "High-value Crop"), alpha = 0.5) +
  labs(title = "Density plot of Efficiency Gap, EE",
       x = "Efficiency Gap",
       y = "Kernel Density") +
  theme_minimal()  + xlim(-0.1, 0.8) + ylim(0, 100)  

G <- ggplot(efficiency, aes(x = tech_gap_se_crop_high, fill = "High-value Crop")) +
  geom_density(alpha = 0.5) +
  #geom_density(aes(x = tech_gap_se_crop_high, fill = "High-value Crop"), alpha = 0.5) +
  geom_density(aes(x = tech_gap_se_arable_crop, fill = "Arable Crop"), alpha = 0.5) +
  labs(title = "Density plot of Efficiency Gap, SE",
       x = "Efficiency Gap",
       y = "Kernel Density") +
  theme_minimal()  #+ xlim(-0.1, 0.5)

ggarrange(E, G, nrow = 2)

# calculate the meta-technology ratio
efficiency$metatechnpology_ratio_ee_urban <- efficiency$eff_ee_meta / efficiency$eff_ee_urban
efficiency$metatechnpology_ratio_se_urban <- efficiency$eff_se_meta / efficiency$eff_se_urban

efficiency$metatechnpology_ratio_ee_rural <- efficiency$eff_ee_meta / efficiency$eff_ee_rural
efficiency$metatechnpology_ratio_se_rural <- efficiency$eff_se_meta / efficiency$eff_se_rural


efficiency$metatechnpology_ratio_ee_intermediate <- efficiency$eff_ee_meta / efficiency$eff_ee_intermediate
efficiency$metatechnpology_ratio_se_intermediate <- efficiency$eff_se_meta / efficiency$eff_se_intermediate

# technology gap
efficiency$tech_gap_ee_urban <- 1 - efficiency$metatechnpology_ratio_ee_urban
efficiency$tech_gap_se_urban <- 1 - efficiency$metatechnpology_ratio_se_urban

efficiency$tech_gap_ee_rural <- 1 - efficiency$metatechnpology_ratio_ee_rural
efficiency$tech_gap_se_rural <- 1 - efficiency$metatechnpology_ratio_se_rural

efficiency$tech_gap_ee_intermediate <- 1 - efficiency$metatechnpology_ratio_ee_intermediate
efficiency$tech_gap_se_intermediate <- 1 - efficiency$metatechnpology_ratio_se_intermediate

# EE: create a density plot of the tech_gap_ee_rural and the tech_gap_ee_intermediate
H <- ggplot(efficiency, aes(x = tech_gap_ee_urban, fill = "urban")) +
  geom_density(alpha = 0.5) +
  geom_density(aes(x = tech_gap_ee_rural, fill = "rural"), alpha = 0.5) +
  geom_density(aes(x = tech_gap_ee_intermediate, fill = "intermediate"), alpha = 0.5) +
  labs(title = "Density plot of Efficiency Gap, EE",
       x = "Efficiency Gap",
       y = "Kernel Density") +
  theme_minimal() # + xlim(-0.1, 0.5)

I <- ggplot(efficiency, aes(x = tech_gap_se_urban, fill = "urban")) +
  geom_density(alpha = 0.5) +
  geom_density(aes(x = tech_gap_se_rural, fill = "rural"), alpha = 0.5) +
  geom_density(aes(x = tech_gap_se_intermediate, fill = "intermediate"), alpha = 0.5) +
  labs(title = "Density plot of Efficiency Gap, SE",
       x = "Efficiency Gap",
       y = "Kernel Density") +
  theme_minimal() # + xlim(-0.1, 0.5)

ggarrange(H, I, nrow = 2)


# calculate the meta-technology ratio
efficiency$metatechnpology_ratio_ee_continental <- efficiency$eff_ee_meta / efficiency$eff_ee_continental
efficiency$metatechnpology_ratio_se_continental <- efficiency$eff_se_meta / efficiency$eff_se_continental

efficiency$metatechnpology_ratio_ee_atlantic <- efficiency$eff_ee_meta / efficiency$eff_ee_atlantic
efficiency$metatechnpology_ratio_se_atlantic <- efficiency$eff_se_meta / efficiency$eff_se_atlantic


efficiency$metatechnpology_ratio_ee_mediterranean <- efficiency$eff_ee_meta / efficiency$eff_ee_mediterranean
efficiency$metatechnpology_ratio_se_mediterranean <- efficiency$eff_se_meta / efficiency$eff_se_mediterranean

# technology gap
efficiency$tech_gap_ee_continental <- 1 - efficiency$metatechnpology_ratio_ee_continental
efficiency$tech_gap_se_continental <- 1 - efficiency$metatechnpology_ratio_se_continental

efficiency$tech_gap_ee_atlantic <- 1 - efficiency$metatechnpology_ratio_ee_atlantic
efficiency$tech_gap_se_atlantic <- 1 - efficiency$metatechnpology_ratio_se_atlantic

efficiency$tech_gap_ee_mediterranean <- 1 - efficiency$metatechnpology_ratio_ee_mediterranean
efficiency$tech_gap_se_mediterranean <- 1 - efficiency$metatechnpology_ratio_se_mediterranean

# EE: create a density plot of the tech_gap_ee_atlantic and the tech_gap_ee_mediterranean
J <- ggplot(efficiency, aes(x = tech_gap_ee_continental, fill = "continental")) +
  geom_density(alpha = 0.5) +
  geom_density(aes(x = tech_gap_ee_atlantic, fill = "atlantic"), alpha = 0.5) +
  geom_density(aes(x = tech_gap_ee_mediterranean, fill = "mediterranean"), alpha = 0.5) +
  labs(title = "Density plot of Efficiency Gap, EE",
       x = "Efficiency Gap",
       y = "Kernel Density") +
  theme_minimal() + xlim(-0.1, 1)

K <- ggplot(efficiency, aes(x = tech_gap_se_continental, fill = "continental")) +
  geom_density(alpha = 0.5) +
  geom_density(aes(x = tech_gap_se_atlantic, fill = "atlantic"), alpha = 0.5) +
  geom_density(aes(x = tech_gap_se_mediterranean, fill = "mediterranean"), alpha = 0.5) +
  labs(title = "Density plot of Efficiency Gap, SE",
       x = "Efficiency Gap",
       y = "Kernel Density") +
  theme_minimal() + xlim(-0.1, 1)

ggarrange(J, K, nrow = 2)

####
# EE, SE combined score
#

# interaction between groups:
# create an indicator that is 1 if `Climatic Zone` == "Mediterranean" and
# specialization == "crop_high", and 0 otherwise
total_data_rm$med_high <- ifelse(total_data_rm$`Climatic Zone` == "Mediterranean" & total_data_rm$specialisation == "High-value Crop", 1, 0)
summary(total_data_rm$med_high == 1)

p_meta + geom_point(data = total_data_rm %>% filter(med_high == 1), color = "red", size = 3) +
  labs(title = "Mediterranean")

total_data_rm$cont_ls <- ifelse(total_data_rm$`Climatic Zone` == "Continental" & total_data_rm$specialisation == "livestock", 1, 0)
summary(total_data_rm$cont_ls == 1)



################

# add identification for the LS regions
efficiency$lsee_meta <- ifelse(efficiency$ls_dum.x  != 0, efficiency$eff_ee_meta, 0)
efficiency$lsse_meta <- ifelse(efficiency$ls_dum.x == 1, efficiency$eff_se_meta, 0)

efficiency$continental <- ifelse(total_data_rm$`Climatic Zone` == "Continental", 1, 0)
efficiency$continental_ee_meta <- ifelse(efficiency$eff_ee_continental != 0, efficiency$eff_ee_meta, 0)
efficiency$continental_se_meta <- ifelse(efficiency$eff_se_continental != 0, efficiency$eff_se_meta, 0)

efficiency$rural <- ifelse(total_data_rm$Typology == "Pred. rural", 1, 0)
efficiency$rural_ee_meta <- ifelse(efficiency$eff_ee_rural != 0, efficiency$eff_ee_meta, 0)
efficiency$rural_se_meta <- ifelse(efficiency$eff_se_rural != 0, efficiency$eff_se_meta, 0)


# meta-plot without labels and with larger dots
livestock_p <- ggplot(efficiency, aes(x = eff_ee_meta , y = eff_se_meta)) +
  geom_point() +
  labs(title = "Meta efficiency", x = "Efficiency EE",
       y = "Efficiency SE") +
  theme_bw() +
  ...
  geom_point(data = efficiency %>% filter(ls_dum.x == 1), color = "black", size = 3) +
  geom_point(data = efficiency, aes(x = eff_ee_livestock, y = eff_se_livestock), 
             size = 3, color = "blue") +
  geom_segment(data = efficiency, 
               aes(x = eff_ee_livestock, y = eff_se_livestock, 
                   xend = lsee_meta, yend = lsse_meta), 
               color = "black")


continental_p <- ggplot(efficiency, aes(x = eff_ee_meta , y = eff_se_meta)) +
  geom_point() +
  labs(title = "Meta efficiency", x = "Efficiency EE",
       y = "Efficiency SE") +
  theme_bw() +
  geom_point(size = 3, color = "grey") + aes(ymin=0)  + aes(xmin=0) +
  xlim(0, 1.02) + ylim(0, 1.02) + 
  geom_point(data = efficiency %>% filter(continental == 1), color = "black", size = 3) +
  geom_point(data = efficiency, aes(x = eff_ee_continental, y = eff_se_continental), 
             size = 3, color = "blue") +
  geom_segment(data = efficiency, 
               aes(x = eff_ee_continental, y = eff_se_continental, 
                   xend = continental_ee_meta, yend = continental_se_meta), 
               color = "black")

rural_p <- ggplot(efficiency, aes(x = eff_ee_meta , y = eff_se_meta)) +
  geom_point() +
  labs(title = "Meta efficiency", x = "Efficiency EE",
       y = "Efficiency SE") +
  theme_bw() +
  geom_point(size = 3, color = "grey") + aes(ymin=0)  + aes(xmin=0) +
  xlim(0, 1.02) + ylim(0, 1.02) + 
  geom_point(data = efficiency %>% filter(rural == 1), color = "black", size = 3) +
  geom_point(data = efficiency, aes(x = eff_ee_rural, y = eff_se_rural), 
             size = 3, color = "blue") +
  geom_segment(data = efficiency, 
               aes(x = eff_ee_rural, y = eff_se_rural, 
                   xend = rural_ee_meta, yend = rural_se_meta), 
               color = "black")




# make this cool plot Hugo talked about
combined <- ggplot() +
  geom_point(data = efficiency, aes(x = eff_ee_livestock, y = eff_se_livestock), 
             size = 3, color = "red") +
  # Points for total_data_rm (meta data)
  geom_point(data = efficiency, aes(x = eff_ee_meta, y = eff_se_meta), 
             size = 3, color = "grey") +
  geom_point(data = efficiency %>% filter(ls_dum == 1), color = "green", size = 3) +
  geom_segment(data = efficiency, 
               aes(x = eff_ee_livestock, y = eff_se_livestock, 
                   xend = eff_ee_meta, yend = eff_se_meta), 
               color = "blue") +
  # Add title and axis labels
  labs(title = "Livestock and Meta Efficiency", 
       x = "Efficiency EE", 
       y = "Efficiency SE") +
  # Set theme
  theme_bw() +
...
combined + geom_point(data = efficiency %>% filter(ls_dum == 1), color = "red", size = 3)



# make this cool plot Hugo talked about
combined <- ggplot() +
  # Points for livestock data
  geom_point(data = livestock, aes(x = eff_ee_lpsolve, y = eff_se_lpsolve), 
             size = 3, color = "red") +
  # Points for total_data_rm (meta data)
  geom_point(data = total_data_rm, aes(x = eff_ee_lpsolve, y = eff_se_lpsolve), 
             size = 3, color = "grey") +
  # Add title and axis labels
  labs(title = "Livestock and Meta Efficiency", 
       x = "Efficiency EE", 
...
efficiency_ee <- efficiency %>% select(NUTS_ID, eff_ee_arable_crop, eff_ee_crop_high, 
                                       eff_ee_livestock, 
                                       eff_ee_continental, eff_ee_atlantic,
                                       eff_ee_mediterranean, eff_ee_urban,
                                       eff_ee_rural, eff_ee_intermediate)

# plot efficiency_ee with a red line going through origo
ggplot(efficiency_ee, aes(x = eff_ee_arable_crop, y = eff_ee_continental)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "red") +
  labs(title = "Efficiency EE", x = "Crop", y = "Continental") +
  theme_bw()

plot(efficiency_ee)

# make a plot using efficiency_ee where NUTS_ids are indexed at the
# x axis and the eff_ee_crop, eff_ee_atlantic and eff_ee_urban are displayed
# at the y axis 

df_long <- efficiency_ee %>%
  pivot_longer(cols = starts_with("eff"), 
               names_to = "group", 
               values_to = "score")

ggplot(df_long, aes(x = NUTS_ID, y = score, color = group, group = group)) +
  geom_point() +          # Add points for better visibility
  theme_minimal() +       # Optional: cleaner theme
  labs(x = "Region ID", y = "Values", title = "Region Values Plot")

#plot(efficiency)

# set NAs to zero
efficiency_ee[is.na(efficiency_ee)] <- 0

# create a column with the highest value for each row
efficiency_ee$high <- apply(efficiency_ee[,2:10], 1, max)

# create a column with the lowest strictly positive value for each row
efficiency_ee$low <- apply(efficiency_ee[,2:10], 1, function(x) min(x[x > 0]))

efficiency_ee$diff <- efficiency_ee$high - efficiency_ee$low

hist(efficiency_ee$diff, breaks = 30)


efficiency_se <- efficiency %>% select(NUTS_ID, eff_se_arable_crop, eff_se_crop_high, 
                                       eff_se_livestock, 
                                       eff_se_continental, eff_se_atlantic,
                                       eff_se_mediterranean, eff_se_urban,
                                       eff_se_rural, eff_se_intermediate)
efficiency_se[is.na(efficiency_se)] <- 0

# create a column with the highest value for each row
efficiency_se$high <- apply(efficiency_se[,2:10], 1, max)

# create a column with the lowest strictly positive value for each row
efficiency_se$low <- apply(efficiency_se[,2:10], 1, function(x) min(x[x > 0]))

efficiency_se$diff <- efficiency_se$high - efficiency_se$low

hist(efficiency_se$diff, breaks = 30)

merge <- full_join(efficiency_ee, efficiency_se, by = "NUTS_ID")

plot(merge$diff.x, merge$diff.y)

# Hist efficiencies

ggplot(efficiency, aes(x = eff_ee_livestock, fill = "EE")) +
  geom_histogram(position = "identity", alpha = 0.8, bins = 15) +
  labs(title = "Histogram of EE in livestock regions",
       x = "EE",
       y = "Frequency") +
  theme_minimal()

# histogram of EE and SE in livestock regions
a <- ggplot(efficiency, aes(x = eff_ee_livestock, fill = "EE", breaks = 10)) +
  stat_bin(alpha = 0.7) +
  #stat_bin(aes(x = eff_se_livestock, fill = "SE"), alpha = 0.5) +
  labs(title = "Histogram of Efficiency in Livestock Regions",
       x = "Efficiency",
       y = "Frequency") + xlim(0, 1.0) + ylim(0, 8)
theme_minimal()

b <- ggplot(efficiency, aes(x = eff_ee_crop_high, fill = "EE")) +
  stat_bin(alpha = 0.7) +
  labs(title = "Histogram of Efficiency in Livestock Regions",
       x = "Efficiency",
       y = "Frequency") + xlim(0, 1.0) + ylim(0, 8)
...
c <- ggplot(efficiency, aes(x = eff_ee_arable_crop, fill = "EE")) +
  stat_bin(alpha = 0.7) +
  labs(title = "Histogram of Efficiency in Livestock Regions",
       x = "Efficiency",
       y = "Frequency") + xlim(0, 1.0) + ylim(0, 8)
...
# density plot of EE and SE in livestock regions
a <- ggplot(efficiency, aes(x = eff_ee_livestock, fill = "EE")) +
  geom_density(alpha = 0.5) +
  geom_density(aes(x = eff_se_livestock, fill = "SE"), alpha = 0.5) +
  labs(title = "Density plot of Efficiency in Livestock Regions",
       x = "Efficiency",
       y = "Kernel Density") + xlim(0, 1.0) +
  theme_minimal()

# density plot of EE and SE in arable_crop regions
b <- ggplot(efficiency, aes(x = eff_ee_arable_crop, fill = "EE")) +
  geom_density(alpha = 0.5) +
  geom_density(aes(x = eff_se_arable_crop, fill = "SE"), alpha = 0.5) +
  labs(title = "Density plot of Efficiency in Arable Crop Regions",
       x = "Efficiency",
       y = "Kernel Density") +
  theme_minimal()

# density plot of EE and SE in crop_high regions
c <- ggplot(efficiency, aes(x = eff_ee_crop_high, fill = "EE")) +
  geom_density(alpha = 0.5) +
  geom_density(aes(x = eff_se_crop_high, fill = "SE"), alpha = 0.5) +
  labs(title = "Density plot of Efficiency in High-value Crop Regions",
       x = "Efficiency",
       y = "Kernel Density") +
  theme_minimal()
